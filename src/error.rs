...
impl<S> From<HandshakeError<S>> for Error {
    fn from(err: HandshakeError<S>) -> Error {
        match err {
            HandshakeError::Failure(e) => From::from(e),
            HandshakeError::Interrupted(_) => Error::Ssl(None),
        }
    }
}
...
impl error::Error for Error {
    ...
    fn cause(&self) -> Option<&error::Error> {
        match *self {
            ...
            #[cfg(feature = "security")]
            Error::Ssl(ref err) => err.as_ref().and_then(|err| err.cause()),
            _ => None,
        }
    }
}

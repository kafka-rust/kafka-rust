...
fn test_consumer_non_existent_topic() {
    // let consumer = test_consumer_builder()
    //     .with_topic_partitions("foo_topic".to_owned(), &TEST_TOPIC_PARTITIONS)
    //     .create();

    // if let Err(consumer_err) = consumer {
    //     let error_code = match consumer {
    //         error::ErrorKind::Kafka(code), _) => code,
    ...
    //     );
    // } else {
    //     panic!("Should have received a Kafka error");
    // }
}
